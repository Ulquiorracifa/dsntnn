...
train_data,_,_ = traf_data.get_data("/home/asprohy/data/traffic")
datatype = 'traf'
# data = train_data
# img_all = []
# label_all = []
...
img = cv2.imread(os.path.join(train_path, train_data['0a2a7e81204b42d3baddd83e187adf4b.jpg']['filepath']))
h, w = img.shape[:2]
img = cv2.resize(img, image_size)
img = np.array(img)
img_tensor = torch.from_numpy(img).permute(2, 0, 1).float()
tmpc = train_data['0a2a7e81204b42d3baddd83e187adf4b.jpg']['bboxes'][0]
img_all = img_tensor
label_all = [int((tmpc['x1'] + tmpc['x2']) / w * image_size[0]), int((tmpc['y1'] + tmpc['y2']) / h * image_size[1])]
